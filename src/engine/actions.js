...
export function allowedActions(c, per, world) {
  ...
  if (world.buildings.shelter.progress < world.buildings.shelter.needed && c.inventory.wood >= 2) push('build_shelter');
  else if (world.buildings.shelter.progress < world.buildings.shelter.needed) push('build_shelter', 'te faltan 2 maderas');
  if (world.buildings.altar.progress < world.buildings.altar.needed && c.inventory.stone >= 1) push('build_altar');
...
export function stepToward(c, world, target, speed = 1) {
  ...
  if (dx) opts.push({ x: c.pos.x + dx, y: c.pos.y });
  if (dy) opts.push({ x: c.pos.x, y: c.pos.y + dy });
  opts.push({ x: c.pos.x + dx, y: c.pos.y + dy });
  opts.push({ x: c.pos.x + (dx || 1), y: c.pos.y + (dy ? 0 : 1) });
  opts.push({ x: c.pos.x, y: c.pos.y + (dy || 1) * -1 });
  for (const o of opts) {
    if (passable(world, o.x, o.y)) { c.pos.x = o.x; c.pos.y = o.y; return true; }
  }
  return false;
}
...
function resolve(sim, c, a) {
  ...
  switch (a.id) {
    ...
    case 'eat': {
      if (inv.fish > 0) { inv.fish--; c.needs.food = clamp(c.needs.food - 40, 0, 100); return finish(sim, c, 'come un pescado'); }
      if (inv.berries > 0) { inv.berries--; c.needs.food = clamp(c.needs.food - 25, 0, 100); return finish(sim, c, 'come bayas'); }
    ...
    case 'forage': {
      ...
      b.amount--; inv.berries += 2;
      return finish(sim, c, 'junta bayas');
    }
    case 'fish': {
      const net = c.blessings.includes('fishing_net');
      if (sim.rng.chance(net ? 0.8 : 0.45)) { inv.fish++; return finish(sim, c, 'pesca un pez'); }
    ...
    case 'build_shelter': {
      ...
      return finish(sim, c, `trabaja en el refugio (${B.progress}/${B.needed})`);
    }
    case 'build_altar': {
      ...
      return finish(sim, c, `apila piedras para el altar (${B.progress}/${B.needed})`);
    }
    case 'pray': {
